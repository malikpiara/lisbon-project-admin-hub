import { resolveCategory } from "@/lib/calendar-categories";
...
function normalizeEvent(item) {
  ...
  const rawTitle = item.summary ?? "(no title)";
  const { categoryId, displayTitle } = resolveCategory(rawTitle);

  return {
    id: item.id,
    title: displayTitle, // category prefix stripped; what the public sees
    rawTitle, // original summary, kept for reference/debugging
    categoryId,
